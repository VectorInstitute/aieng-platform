...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!code || !state) {
      return NextResponse.redirect(`${baseUrl}/analytics?error=invalid_callback`);
    }
    ...
    if (!verifier) {
      return NextResponse.redirect(`${baseUrl}/analytics?error=missing_verifier`);
    }
    ...
    if (authConfig.allowedDomains && authConfig.allowedDomains.length > 0) {
      ...
      if (!domain || !authConfig.allowedDomains.includes(domain)) {
        return NextResponse.redirect(`${baseUrl}/analytics?error=unauthorized_domain&domain=${domain}`);
      }
    }
    ...
    return NextResponse.redirect(`${baseUrl}/analytics?error=authentication_failed`);
  }
}
